...
| **Systerbolagets domän är nyss driftsatt** | CM-uppdrag-kortet på `/tjanster` (`src/components/pages/ServicesPage.jsx`) länkar till <https://byggoprojektgruppen.se>, systerbolaget Bygg & Projektgruppen i Stockholm AB. Domänen är nyss driftsatt och svarade `200` när den kontrollerades senast. | Skulle länken vara nere vid leveransen är det tillfälligt och löser sig när domänen är uppe — ingen åtgärd behövs i den här koden. Skulle adressen bli en annan byts den på ett ställe, raden `href` i CM-kortets objekt. |
